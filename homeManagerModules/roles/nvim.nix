...
in {
  ...
  options.myOptions.roles.nvf = {
    ...
    notesPackage = lib.mkOption {
      description = "Stripped down package containing only notes utilities.";
      type = lib.types.package;
    };
    defaultNvfSettings = lib.mkOption {
      description = "Attrset of nvf settings, before it gets processed by nvf. This avoids conflicts on e.g. build.finalPackage when overriding the configuration";
    ...
    myOptions.roles.nvf = let
      ...
      nvfRemoveBuildConficts = nvfIn:
        lib.attrsets.updateManyAttrsByPath [
          {
            path = ["build"];
            update = old: lib.attrsets.removeAttrs old ["finalPackage"];
          }
          {
            path = ["treesitter"];
            update = old: lib.attrsets.removeAttrs old ["defaultGrammars"];
          }
        ]
        nvfIn;
    in {
      ...
      notesPackage =
        (inputs.nvf.lib.neovimConfiguration {
          ...
          modules = [
            {
              config.vim = lib.attrsets.recursiveUpdate (config.myOptions.roles.nvf.defaultNvfSettings) {
                viAlias = false;
                vimAlias = false;
                lsp = {
      ...
      defaultNvfSettings = {
        ...
        utility = {
          ...
          surround.enable = true; # TODO: Lookup current bindings
